...
<body>
	...
	<div class="frame-main">
		<div style="height:8%; padding-left:8%; background: rgb(164,205,226);opacity:1;padding-top:3%">
			<!--  <input id = "upload1" type="button" style="color:black ; margin:10px" value="上传文件"/>
		 	<input type="button" style="color:black ; margin:10px" value="新建文件夹"/> -->
			<div id="upload1" >
				<form action= "uploadPro" method="post" enctype="multipart/form-data">
					...
					<input onclick="javascript: return checkFile()" type="submit" value="上传"/>
		...
		<c:forEach items = "${requestScope.list }" var="file">
			<div class="list_m" style="width:100%">
			 	<span class="list_main" style="width:30%">
					...
					<img class="sign" src="Images/wenjian.jpg"/>
	...
	<script>
	$("#curren").onchange(function(){
		$.ajax({
			type:"POST",
			url:"/Files/file_findByPath.action",
			data: {username:$("#username").val(), savePath:$("#currentPath").val()},
			dataType: "json",
			success:function(data){
				$("#list-all").empty();
				var html=data.$("#list-all").innerHTML;
				doucument.getElementById("list-all").innerHTML=html;
			} 
		});
	});
	...
	function checkFile(){
		var s = document.getElementById("uploadFile").value;
		if(s ==null || s=="")
		{
			alert("目前还未选择上传文件！");
			return false;
